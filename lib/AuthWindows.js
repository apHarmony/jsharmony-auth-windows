...
var VerifyConfig = function(jsh, config){
...
var AuthWindows = function(jshsite, config){
  ...
  var validatePassword = function(req, jsh, user_info, password, cb){
    ...
    if (config.debug_params.log_auth_timing) console.time('validatePassword'); // eslint-disable-line no-console
    ...
    AuthenticateUser({
      ...
      if (config.debug_params.log_auth_timing) console.timeEnd('validatePassword'); // eslint-disable-line no-console
  ...
  var getTrustedToken = function(req, jsh, user_info, cb){
    ...
    */
    ...
    if (config.debug_params.log_auth_timing) console.time('getTrustedToken'); // eslint-disable-line no-console
    ValidateUser({
      domain_controller: config.domain_controller,
      ...
      if (config.debug_params.log_auth_timing) console.timeEnd('getTrustedToken'); // eslint-disable-line no-console
      if (error) return cb(error);
      if (pwdLastSet > 0){
        var token = crypto.createHash('sha1').update(windows_account + pwdLastSet + jshsite.auth.salt).digest('hex');
        session.accessTime = Date.now();
