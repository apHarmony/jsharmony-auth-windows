...
var AuthWindows = function(config){
  VerifyConfig(config);
  var auth_salt = config.mainsalt;
  var cache = {};

  var authenticate = function(req, jsh, user_info, cb){
  ...
  var validate = function(req, jsh, user_info, cb){
    var userPrincipalName = user_info['sys_user_windows_account'];
    ...
    if (config.debug_params.log_auth_timing) console.time('validate');
    ValidateUser({
      domain_controller: config.domain_controller,
      ...
      if (config.debug_params.log_auth_timing) console.timeEnd('validate');
      if (error) return cb(error);
      if (pwdLastSet != 0){
        var token = crypto.createHash('sha1').update(userPrincipalName + pwdLastSet + auth_salt).digest('hex');
        cache[userPrincipalName] = {
  ...
  return {
    ...
    on_superauthenticate: authenticate,
